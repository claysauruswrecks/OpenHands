...
function handleVSCodeFileOpen(message: ObservationMessage) {
  ...
  if (!filePath) {
    // VSCode file open observation missing file_path
    return;
  }

  // Opening file in VSCode
  ...
  if (!conversationIdMatch) {
    // Could not extract conversation ID from URL
    return;
  }
      ...
      if (iframe && iframe.contentWindow) {
        ...
        try {
          ...
          if (iframeDoc) {
            ...
            // Successfully opened file using command URI
          } else {
            ...
            // Opened file via iframe URL modification
          }
        } catch (iframeError) {
          // Command URI method failed, trying URL parameter
          ...
          // Opened file via iframe URL fallback
        }
      } else {
        // VSCode iframe not found or no contentWindow available
      }
    } catch (error) {
      // Failed to open file in VSCode
    }
