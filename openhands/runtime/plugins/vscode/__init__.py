...
class VSCodePlugin(Plugin):
    ...
    async def initialize(self, username: str) -> None:
        ...
        cmd = (
            ...
            f"cd {workspace_path}\n"
            f"exec /openhands/.openvscode-server/bin/openvscode-server --host 0.0.0.0 --connection-token {self.vscode_connection_token} --port {self.vscode_port} --extensions-dir /openhands/.openvscode-server/extensions --disable-workspace-trust\n"
            "EOF"
        )
    ...
    def _setup_vscode_settings(self) -> None:
        ...
        and copying the settings.json file there, with dynamic port configuration.
        """
        ...
        # Dynamically set the remoteControl.port from environment variable
        vscode_remote_control_port = os.getenv("VSCODE_REMOTE_CONTROL_PORT")
        if vscode_remote_control_port:
            try:
                settings["remoteControl.port"] = int(vscode_remote_control_port)
                logger.debug(
                    f"Set vscode-remote-control port to {vscode_remote_control_port}"
                )
            except ValueError:
                logger.warning(
                    f"Invalid VSCODE_REMOTE_CONTROL_PORT value: {vscode_remote_control_port}, using default"
                )
                settings["remoteControl.port"] = 3710
        else:
            # Fallback to default port if environment variable is not set
            settings["remoteControl.port"] = 3710

        # Ensure host is set to 0.0.0.0 to bind to all interfaces (allows external connections)
        settings["remoteControl.host"] = "0.0.0.0"

        # Write the updated settings to the .vscode directory
        ...
        logger.debug(
            f"VSCode settings with dynamic port configuration written to {target_path}"
        )
